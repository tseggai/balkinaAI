-- Migration 007: Phase 6 — Full Feature Parity
...
-- ── Service enhancements (new columns not already in 006) ─────────────────────
...
-- ── Service staff enhancements (add pricing columns) ──────────────────────────
...
BEGIN
  FOR tbl IN SELECT unnest(ARRAY[
    'custom_fields', 'packages', 'loyalty_programs', 'products', 'staff_roles'
  ])
  LOOP
    EXECUTE format(
      'CREATE POLICY IF NOT EXISTS "Tenant manages %1$s" ON %1$s FOR ALL TO authenticated USING (tenant_id IN (SELECT id FROM tenants WHERE user_id = auth.uid()))',
      tbl
    );
  END LOOP;
END $$;

-- Junction/child table RLS via parent
CREATE POLICY IF NOT EXISTS "Tenant manages appointment_custom_field_values"
...
CREATE POLICY IF NOT EXISTS "Tenant manages staff_holidays"
...
CREATE POLICY IF NOT EXISTS "Tenant manages package_services"
...
CREATE POLICY IF NOT EXISTS "Tenant manages customer_packages"
  ON customer_packages FOR ALL TO authenticated
  USING (tenant_id IN (SELECT id FROM tenants WHERE user_id = auth.uid()));

CREATE POLICY IF NOT EXISTS "Tenant manages loyalty_rules"
...
CREATE POLICY IF NOT EXISTS "Tenant manages customer_loyalty_points"
  ON customer_loyalty_points FOR ALL TO authenticated
  USING (tenant_id IN (SELECT id FROM tenants WHERE user_id = auth.uid()));

CREATE POLICY IF NOT EXISTS "Tenant manages loyalty_transactions"
  ON loyalty_transactions FOR ALL TO authenticated
  USING (tenant_id IN (SELECT id FROM tenants WHERE user_id = auth.uid()));

CREATE POLICY IF NOT EXISTS "Tenant manages product_services"
...
CREATE POLICY IF NOT EXISTS "Tenant manages staff_role_assignments"
...
CREATE POLICY IF NOT EXISTS "Tenant manages role_permissions"
