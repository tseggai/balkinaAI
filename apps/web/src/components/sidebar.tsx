...
const navItems = [
  ...
  { href: '/dashboard/roles', label: 'Roles', icon: ShieldIcon },
  ...
  useEffect(() => {
    ...
    const fetchUnread = () => {
      fetch('/api/tenant/messages')
        ...
        .then((j) => { if (!cancelled) setUnread(j.unread_count ?? 0); })
        .catch(() => {});
    };
    fetchUnread();
    window.addEventListener('tenant-messages-read', fetchUnread);
    return () => { cancelled = true; window.removeEventListener('tenant-messages-read', fetchUnread); };
  }, [pathname]);
    ...
    <aside className="flex h-full flex-col border-r border-gray-200 bg-white">
      ...
      <div className="border-t border-gray-200 p-4">
        <div className="mb-3">
          ...
          <span className="mt-0.5 inline-block rounded-full bg-brand-100 px-2 py-0.5 text-xs font-medium text-brand-700">
            {planName}
          </span>
        </div>
        <button
...
function UserGroupIcon({ active }: { active?: boolean }) {
  return (
    <svg className={`h-5 w-5 ${active ? 'text-brand-600' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
