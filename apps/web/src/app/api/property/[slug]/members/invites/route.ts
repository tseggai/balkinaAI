...
/** Email + SMS an invite. Best-effort per channel; returns which succeeded. */
async function deliver(property: Prop, c: CodeRow): Promise<{ email: boolean; sms: boolean }> {
  const typeLabel = memberTypeLabel(c.member_type);
  const out = { email: false, sms: false };

  const storeLinks = [
    property.app_store_url ? `<a href="${property.app_store_url}" style="color:#2563eb">App Store</a>` : null,
    property.play_store_url ? `<a href="${property.play_store_url}" style="color:#2563eb">Google Play</a>` : null,
  ].filter(Boolean).join(' · ');
  ...
  if (c.email) {
    try {
      await sendEmail({
        to: c.email,
        fromName: `${property.name} (via Balkina AI)`,
        ...
        html: `<div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px">
          ...
          <p style="color:#555;margin-top:18px">Open the app, tap <strong>Verify your residence</strong>, and enter this code:</p>
          ...
          ${storeLinks ? `<p style="color:#999;font-size:13px">Get the app: ${storeLinks}</p>` : ''}
        </div>`,
        text: `Welcome to ${property.name}. You've been invited as a ${typeLabel}${c.unit ? ` (unit ${c.unit})` : ''}. Open the ${property.name} app, tap "Verify your residence", and enter code ${c.code}.`,
      });
      out.email = true;
...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (body.resendId) {
    ...
    const delivery = await deliver(property, c);
  ...
  const delivery = await deliver(property, { code, member_type, email, phone, unit });
